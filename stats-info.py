...
def getStatsInfo(path2InDir, path2OutDir, corpus):
    ...
    with open(os.path.join(path2OutDir, corpus + '.info'), 'w') as fwrite:
        ...
        fwrite.write('Number of documents: %d\n' % nbDocuments)
        fwrite.write('Vocabulary size of contents: %d\n' % len(dictWordsAllConts))
        fwrite.write('Vocabulary size of highlights: %d\n' % len(dictWordsAllHglights))
        fwrite.write('Average words in the content: %.2f\n' % avgWordAllConts)
        fwrite.write('Average words in the highlights: %.2f\n' % avgWordAllHglights)
        fwrite.write('Average sentences in the content: %.2f\n' % avgSentAllConts)
        fwrite.write('Average sentences in the highlights: %.2f\n' % avgSentAllHglights)
        fwrite.write('Average new words in the highlights: %.2f\n' % avgDiffHglightsVsContent)
        fwrite.write('Maximum number of words in the content: %d\n' % np.max(lstNbWordsConts))
        fwrite.write('Maximum number of words in the highlights: %d\n' % np.max(lstNbWordsHglights))
        fwrite.write('Minimum number of words in the content: %d\n' % np.min(lstNbWordsConts))
        fwrite.write('Minimum number of words in the highlights: %d\n' % np.min(lstNbWordsHglights))
        fwrite.write('========================================================\n')
        fwrite.flush()
