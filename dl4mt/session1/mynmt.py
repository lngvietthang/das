...
def prepare_data(seqs_x, seqs_y, maxlen_src= None, maxlen_trg = None ,n_words_src=30000, n_words=30000):
    ...
    if maxlen_src is not None and maxlen_trg is not None:
        new_seqs_x = []
        new_seqs_y = []
        new_lengths_x = []
        new_lengths_y = []
...
def build_model(tparams, options):
    ...
    trng = RandomStreams(4488)
    use_noise = theano.shared(np.float32(0.))

    # description string: #words x #samples
    # x, x_mask: shape (n_in_step, minibatch)
    # y, y_mask: shape (n_out_step, minibatch)
    ...
    # emb: shape (n_in_step, minibatch, dim_word)
    ...
    # proj1[0]: shape (nstep, minibatch, hidden_dim)  -- because return [proj] so get [0]. i dont know why the author do this
    ...
    # ctx: shape (minibatch, hidden_dim)
    ctx = proj1[0][-1]
    ctx_mean = ctx

    # initial decoder state
    # init_shape: shape (minibatch, hidden_dim)
    ...
    # emb: shape (n_out_step, minibatch, dim_word)
    ...
    # proj, shape: (n_out_step, minibatch, him_hidden) , because this layer return proj not [proj] no we dont need to get [0]
    ...
    # just add 1 dimention to first
    # ctxs: shape (1, minibatch, him_hidden)
    ctxs = ctx[None, :, :]

    # compute word probabilities
    # logit_element:    output each steps of decoder
    #                   input each steps of decoder
    #                   last steps of encoder
    # shape: (n_out_step, minibatch, embsize)
    ...
    # logit shape: (n_out_step, minibatch, vocab_target_size)
    logit = tensor.tanh(logit_lstm+logit_prev+logit_ctx)
    logit = get_layer('ff')[1](tparams, logit, options, prefix='ff_logit',activ='linear')


    # softmax shape: (nstep * minibatch, vocab_target_size)
    logit_shp = logit.shape
    probs = tensor.nnet.softmax(logit.reshape([logit_shp[0]*logit_shp[1], logit_shp[2]]))


    # cost --------------- this right because i try it in main.py
    y_flat = y.flatten()
    y_flat_idx = tensor.arange(y_flat.shape[0]) * options['n_words'] + y_flat
    cost = -tensor.log(probs.flatten()[y_flat_idx])
    cost = cost.reshape([y.shape[0], y.shape[1]])
    cost = (cost * y_mask).sum(0) # Multi mask to not affect to cost

    return trng, use_noise, x, x_mask, y, y_mask, opt_ret, cost, logit,
...
def pred_probs(f_log_probs, prepare_data, options, iterator, verbose=True):
    ...
    for x, y in iterator:
        ...
        x, x_mask, y, y_mask = prepare_data(x, y, maxlen_src=options["maxlen_src"], maxlen_trg=options["maxlen_trg"],
                                            n_words_src=options["n_words_src"],n_words=options["n_words"])
...
def train(dim_word=100,  # word vector dimensionality
          ...
          maxlen_src=300,  # maximum length of the description
          maxlen_trg=75,
          optimizer='adadelta',
          batch_size=16,
          valid_batch_size=16,
          ...
          datasets=['/data/lisatmp3/chokyun/europarl/europarl-v7.fr-en.en.tok','/data/lisatmp3/chokyun/europarl/europarl-v7.fr-en.fr.tok'],
          valid_datasets=['../data/dev/newstest2011.en.tok','../data/dev/newstest2011.fr.tok'],
          dictionaries=['/data/lisatmp3/chokyun/europarl/europarl-v7.fr-en.en.tok.pkl','/data/lisatmp3/chokyun/europarl/europarl-v7.fr-en.fr.tok.pkl'],
          use_dropout=False,
          reload_=False,
          overwrite=False):
    ...
    for x, y in train:
        n_samples += len(x)
        A,B,C,D = prepare_data(x, y, maxlen_src=maxlen_src, maxlen_trg=maxlen_trg,n_words_src=n_words_src,n_words=n_words)
        break
    ...
    # hyhy = theano.function([x, x_mask, y, y_mask],[cost,lstm1])
    # xxx,yyy = hyhy(A,B,C,D)
    ...
    for eidx in xrange(max_epochs):
        ...
        for x, y in train:
            ...
            x, x_mask, y, y_mask = prepare_data(x, y, maxlen_src=maxlen_src, maxlen_trg=maxlen_trg,n_words_src=n_words_src,n_words=n_words)

            if x is None:
                print 'Minibatch with zero sample under src length ', maxlen_src, "trg ", maxlen_trg
                uidx -= 1
                continue
            ...
            if np.mod(uidx, saveFreq) == 0:
                ...
                np.savez(saveto, history_errs=history_errs, uidx=uidx, **params)
            ...
            if np.mod(uidx, sampleFreq) == 0:
                ...
                for jj in xrange(np.minimum(5, x.shape[1])):
                    stochastic = True
            ...
            if np.mod(uidx, validFreq) == 0:
                ...
                if uidx == 0 or valid_err <= np.array(history_errs).min():
                    best_p = unzip(tparams)
                    bad_counter = 0
                if len(history_errs) > patience and valid_err >= \
                        np.array(history_errs)[:-patience].min():
                    bad_counter += 1
    ...
    np.savez(saveto, zipped_params=best_p,
...
def main(job_id, params):
    ...
    validerr = train(saveto=params['model'][0],
                                        ...
                                        batch_size=32,
                                        valid_batch_size=32,
					datasets=['../data/all_content-summary.content.tok',
					'../data/all_content-summary.summary.tok'],
					valid_datasets=['../data/newstest2011.content.tok',
					'../data/newstest2011.summary.tok'],
					dictionaries=['../data/all_content-summary.content.tok.bpe.pkl',
					"../data/all_content-summary.summary.tok.bpe.pkl"],
                                        validFreq=5000,
                                        dispFreq=10,
                                        saveFreq=1000,
                                        sampleFreq=100,
