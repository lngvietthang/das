...
/usr/bin/perl $P1/tokenizer.perl -threads 4 -l en < ${P1}/all_${S}-${T}.${S} > ${P1}/all_${S}-${T}.${S}.tok
/usr/bin/perl $P1/tokenizer.perl -threads 4 -l en < ${P1}/all_${S}-${T}.${T} > ${P1}/all_${S}-${T}.${T}.tok
/usr/bin/perl $P1/tokenizer.perl -threads 4 -l en < ${P1}/test2011/newstest2011.${S} > ${P1}/newstest2011.${S}.tok
/usr/bin/perl $P1/tokenizer.perl -threads 4 -l en < ${P1}/test2011/newstest2011.${T} > ${P1}/newstest2011.${T}.tok


# BPE
if [ ! -f "${S}.bpe" ]; then
    python2.7 $P2/learn_bpe.py -s 20000 < all_${S}-${T}.${S}.tok > ${S}.bpe
fi
if [ ! -f "${T}.bpe" ]; then
    python2.7 $P2/learn_bpe.py -s 20000 < all_${S}-${T}.${T}.tok > ${T}.bpe
fi

# utility function to encode a file with bpe
encode () {
    if [ ! -f "$3" ]; then
        python2.7 $P2/apply_bpe.py -c $1 < $2 > $3
    else
        echo "$3 exists, pass"
    fi
}
...
# shuffle
python2.7 $P1/shuffle.py all_${S}-${T}.${S}.tok.bpe all_${S}-${T}.${T}.tok.bpe

# build dictionary
python2.7 $P1/build_dictionary.py all_${S}-${T}.${S}.tok.bpe
python2.7 $P1/build_dictionary.py all_${S}-${T}.${T}.tok.bpe